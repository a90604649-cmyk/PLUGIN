package com.savagepvp.prueba.command;
...
public final class PruebaCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleSave(CommandSender sender, String[] args) {
        ...
        service.save(key, value).whenComplete((ignored, throwable) ->
                runOnMain(sender, () -> {
                    if (throwable != null) {
    ...
    private boolean handleView(CommandSender sender, String[] args) {
        ...
        String key = args[1];
        service.find(key).whenComplete((result, throwable) ->
                runOnMain(sender, () -> {
                    if (throwable != null) {
                    ...
                    MessageUtil.send(sender, "&eValue: &f" + entry.getValue());
                    MessageUtil.send(sender, "&6&m------------------------------");
                }));
        return true;
    }
    ...
    private boolean handleList(CommandSender sender) {
        ...
        service.findAll().whenComplete((entries, throwable) ->
                runOnMain(player, () -> {
                    if (throwable != null) {
    ...
    private void runOnMain(CommandSender sender, Runnable task) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            player.getServer().getScheduler().runTask(listGui.getPlugin(), task);
        } else {
            // Console callbacks are still marshalled through Bukkit's scheduler.
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            for (String option : options) if (option.startsWith(args[0].toLowerCase())) result.add(option);
            return result;
        }
